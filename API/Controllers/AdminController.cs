    ...
    IPhotoService photoService) : BaseApiController
{
    ...
    public async Task<ActionResult<IList<string>>> EditRoles(string userId, [FromQuery] string roles)
    {
        ...
        if (user == null) return BadRequest("Cannot find the user to edit role.");
        ...
        if (!result.Succeeded) return BadRequest("Failed to remove unselect roles.");
    ...
    public async Task<ActionResult> ApprovePhoto(int photoId)
    {
        ...
        if (member == null) return BadRequest("Could not get member from Database");
