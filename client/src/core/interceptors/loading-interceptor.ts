...
export const loadingInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  if (req.method.includes('GET') && req.url.includes('/members/')) {
  ...
  return next(req).pipe(
    ...
    // identity: we do not give it the parameter, so it's effectively null.
    // Instead of using 'null' directly --> this would be showed warning
    tap(response => {
      cache.set(cacheKey, {
        response,
        timestamp: Date.now()
