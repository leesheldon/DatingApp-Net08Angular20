...
export class MessageService {
  ...
  createHubConnection(otherUserId: string) {
    ...
    this.hubConnection.start().catch(errror => console.log(errror));

    this.hubConnection.on('ReceiveMessageThread', (response: Message[]) => {
      this.messageThread.set(response.map(message => ({
          ...message,
